...
given_args = ~cellfun('isempty',varargin);% apply functions to each cell in the cell array
defaults(given_args) = varargin(given_args);
[reltol,quiet,eta,pcgmaxi,x,u,t] = deal(defaults{:});% copies the single input to all the requested outputs
...
for ntiter = 0:MAX_NT_ITER
    ...
    maxAnu = norm(At*nu,inf);%Newton method 
    if (maxAnu > lambda)
        nu = nu*lambda/maxAnu;
    end
    
    pobj  =  z'*z+lambda*norm(x,1);% the value of loss function
